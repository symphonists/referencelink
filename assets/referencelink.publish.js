...
var ReferenceLink = {
	...
	init: function() {
		...
		jQuery(".reflink_search").each(function(n) {
			...
			if(jQuery(this).attr('multi') == 'yes') {
				self.multiple[i] = true;
			} else {
				self.multiple[i] = false;
			}

			// Initialize selection list
			self.selected[i] = new Array();

			// Load preexisting selections
			jQuery("#reflink_selections" + i).children("li").each(function(n) {
				self.selected[i][n] = { name: jQuery(this).attr('class'), id: jQuery(this).attr('id') };
			});
	...
	/*
	 * Builds the comma-separated list of values for submission
	 */
	...
	buildSelectionList: function(sel, n) {
		...
		// Append a list item for each selected value
		for (i in sel) {
			if (sel[i].id != undefined) {
		...
		// If there are no selections, make sure the field's showing
		if (sel.length === 0) {
			jQuery("#reflink_search" + n).show();
		}
		// Bind function to remove a selection to the list
		jQuery("a.deselect").bind("click", sel, function(){
			...
			// Update the submittable input
			jQuery("#reflink_input" + n).val(self.getSelectedValues(sel));
